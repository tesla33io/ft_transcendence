...
export class Migration20251028134149 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table \`user_friends\` (\`user_1_id\` integer not null, \`user_2_id\` integer not null, constraint \`user_friends_user_1_id_foreign\` foreign key(\`user_1_id\`) references \`user\`(\`id\`) on delete cascade on update cascade, constraint \`user_friends_user_2_id_foreign\` foreign key(\`user_2_id\`) references \`user\`(\`id\`) on delete cascade on update cascade, primary key (\`user_1_id\`, \`user_2_id\`));`);
    this.addSql(`create index \`user_friends_user_1_id_index\` on \`user_friends\` (\`user_1_id\`);`);
    this.addSql(`create index \`user_friends_user_2_id_index\` on \`user_friends\` (\`user_2_id\`);`);
